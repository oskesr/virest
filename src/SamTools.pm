...
sub run {
    ...
    my $vcf=$aln;
    $vcf=~s/.sam/.vcf/;
    $vcf=~s/bwaalign/variants/;

    my $localalnsam=$s3_utils->s3get($aln);
    my $localreffna=$s3_utils->s3get($reffna);
    my $localalnbam=File::Temp->new;
    my $localalnbamsort=File::Temp->new;;
    my $localvcf=File::Temp->new;
       
    system( "samtools view -bS -o $localalnbam $localalnsam && samtools sort $localalnbam alnbs -o >$localalnbamsort && samtools mpileup -uf $localreffna $localalnbamsort | bcftools view -vcg - > $localvcf") && die qq(error SamTools failed);
    
    $s3_utils->s3put($localvcf,$vcf);
    $self->dataflow_output_id({ vcf => $vcf }, 1);
    #print $message;
    return 1;
}
